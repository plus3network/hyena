...
function Query(model, options) {
  ...
  this.data = { 
    type: 'SELECT', 
    alias: this.options.useAlias && aliasify(this.model.table) || null,
    limit: null, 
    order: [], 
    selects: { },
    fields: [], 
...
Query.prototype.upsert = function (doc) {
  ...
  this.data.fields.push(_.without(keys, 'id').map(function (key) {
    self.data.values.push(doc[key]);
    return self.escapeId(self.table, key)+' = ?';
  }).join(', '));
  return this;
...
Query.prototype.fields = function () {
  ...
  tables.forEach(function (table) {
    fields.push(self.escapeId(table, 'id')+' AS '+table+'_id');
...
Query.prototype.processPopulates = function () {
  ...
  this.data.population = newPop;
    ...
    self.select(aliasify(path), self.data.pathFields[path]);
		self.join(model.table, 'id', value.joinType || 'inner', aliasify(path)).on(value.parent, value.field);
