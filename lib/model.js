...
var manyToMany = function (doc, def, options, callback) {
  ...
  query
    .join(def.schema.through, def.schema.foriegn_key, 'INNER', aliasify(options.path))
    .on(aliasify(model.table), 'id')
...
var oneToMany = function (doc, def, options, callback) {
  var model = this.db.model(def.schema.type);
  var query = model.find().where(def.schema.foriegn_key).equals(doc.id);
  var match = options.match || def.schema.match;
  if (options.select) {
    query.select(options.select);
  }
...
Model.populate = function (doc, path, callback) {
  ...
  // If the schema type is an array then we have a many-to-many or one-to-many
  // relationsship and we need to handle it differently then the one-to-one
  if (def.type === 'array') {
    // many to many
    if (def.schema.through) {
      manyToMany.call(this, doc, def, options, callback);
    }
    // one to many
    if (def.schema.foriegn_key && !def.schema.through) {
      oneToMany.call(this, doc, def, options, callback);
    }
  } else {
    oneToOne.call(this, doc, def, options, callback);
  }
};
