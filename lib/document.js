...
var buildGetterAndSetters = function () {
  ...
  // set defaults
    ...
    if (def && def.type === 'array' && def.schema.through) {
      // Initalize with an empty collection
      self.attributes[key] = new Collection(def, self, []);
      def.set = function (v) {
        return new Collection(def, this, v); 
      };
    }
  ...
  _.forIn(schema.virtuals, function (value, path) {
    Object.defineProperty(self, path, {
      get: value.get.bind(self),
      set: value.set.bind(self)
    });
  });
...
Document.prototype.remove = function (callback) {
  ...
  this.runPre('remove', [this], function (err) {
    ...
    query.delete().where('id').equals(self.id).exec(function (err, info) {
      if (err) {
        handleError.call(self, err);
        return callback(err);
      }
