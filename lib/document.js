		...
		Object.defineProperty(self, key, {
			...
			set: function (value) {
              ...
              addSimpleGetterSetter(self, def.field);
						...
						self.attributes[def.field] = value.id;
					}
...
Document.prototype.save = function (callback) {
	...
	this.validate(function (err) {
		...
		var query = new Query(self.constructor);
...
Document.prototype.remove = function (callback) {
  var self = this;
  var query = new Query(this.constructor);
  ...
  this.runPre('remove', [this], function (err) {
    ...
    query.delete().where('id').equals(this.id).exec(function (err, info) {
      if (err) {
        handleError.call(self, err);
        return callback(err);
      }
