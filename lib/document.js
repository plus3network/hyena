...
Document.prototype.hydrate = function (doc) {
  ...
  var saveOrig = this.constructor.schema.options.saveOrigValues;
  
  var data = _.defaults(doc, model.data);
  var clone_of_orig = {}; //shallow with no ref to the orig values
	var self = this;

  if (saveOrig) {
    if (!self.orig)  self.orig = {};
    clone_of_orig = _.clone(data);
  }  
	var keys = Object.keys(data);
	keys.forEach(function (key) {
    if (saveOrig) {
      // console.log("Saving orig value on doc id "+ doc.id + ": "+ key +" => "+ clone_of_orig[key]);
      self.orig[key] = clone_of_orig[key];
    }
		self[key] = data[key];
    if(key !== self.idKey) self.pathStates.reset(key);
	});
  if(saveOrig) Object.freeze(self.orig);
};
